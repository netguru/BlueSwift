...
extension Logger {
    ...
    func log(_ type: LogType, _ event: @autoclosure () -> Event, data: Any? = nil, category: String = LogCategory.blueSwift.rawValue, file: String = #fileID, function: String = #function) {
...
public extension Logger {
    ...
    func mapEvent(_ transform: @escaping ((@autoclosure () -> Event), EventContext) -> Event) -> Logger {
        AnyLogger { event, context in
    ...
    /// - Parameter transform: transform function.
    /// - Returns: a new logger which transforms events.
    func mapContext(_ transform: @escaping ((@autoclosure () -> Event), EventContext) -> EventContext) -> Logger {
        AnyLogger { event, context in
    ...
    func flatMap(_ transform: @escaping ((@autoclosure () -> Event), EventContext) -> Logger) -> Logger {
        AnyLogger { event, context in
    ...
    func filter(_ predicate: @escaping ((@autoclosure () -> Event), EventContext) -> Bool) -> Logger {
        AnyLogger { event, context in
